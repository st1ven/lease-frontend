<template>
	<view style="overflow-x: hidden;">
		...
		<view class="info">
			<image class="info-bg" src="" mode=""></image>
			
			<view class="info-content">
				<view class="info-avatar">
					<image class="info-avatar-pic" :src="avatar" mode="widthFix"></image>
				</view>
				<view class="info-name">
					<text class="info-name">{{nickName}}</text>
					<text class="info-level">{{nickName}}</text>
				</view>
			</view>
		...
		<view class="button update" @click="onOpenCard" :style="{backgroundImage:'url('+membercenter.memberimage.kaika_image+')'}">
		...
		<view class="privilege">
			...
			<view class="privilege-item" v-for="(item, index) in membercenter.membercenter" :key="index" hover-class="hover" @click="quanyitiao(item)">
				<image class="privilege-item-pic" :src="item.image" mode="widthFix" ></image>
			</view> 
		</view>
		<!-- 商品 -->
		<view class="" :style="{backgroundImage:'url('+membercenter.memberimage.goods_image+')'}" style="position: relative;background-size: 100% 100%;">
			<view class="" style="color: #ffb468;font-size: 16px;text-align: right;padding: 20px;" @click='JumpHot({indexFlag:3,id:47})'>
				查看完整精选 >
			</view>
			<view class="iz-goods-tpl" :style="'background-color:'+Bgc || '#f0f0f0'">
			<view class="iz-goods-item" v-for="(item,index) in hotList" :key="index" :style="itemStyle"
		...
		<view v-for="(item,index) in membercenter.memberimage.images">
...
<style lang="scss">
	...
	.level {
		...
		width: 750rpx;
		height: 466rpx;
		background-image: url('https://oss.jiulove.cn/uploads/20220801/2b5c800370debabc81faa2233714e73c.png');
		background-size: 100%;
