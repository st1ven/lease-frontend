	...
	<view class="address-page">
		<view class="address-ul">
			<view v-for="(item,inx) in list" :key="inx" @click.self="liClick(inx)"
				...
				<text class="edit icon" @click="edit({type:1,data:item})">
					<u-icon name="edit-pen" color="#909399" size="48rpx"></u-icon>
				</text>
				<text class="del icon" @click="delShow = true">
		...
		<u-popup :show="editShow" mode="center" :borderRadius="'30rpx'" :closeIconPos="'top-right'"
			...
			<view class="slot-content addressModel">
				...
				<view class="input-ul">
					...
					<view class="input-li">
						<view class="input-label">* 紧急联系人：</view>
						<view class="input-box" style="width: 60%;">
							<u-input placeholder="请输入" :border="'surround'" fontSize="24rpx"
								v-model="formObj.urgent_name"></u-input>
						</view>
						<view class="color-bule" @click="showSex=true">
							{{formObj.relationship || "关系"}}
							<view class="icon-box">
								<u-icon  name="arrow-down" size="30rpx" color="#ddd"></u-icon>
							</view>
						</view>
						<view class="relationship-ul" v-if="showSex">
							<view class="relationship-li" v-for="(item,index) in actions" :key="index"
								@click="sexSelect(item)">
								{{item}}
							</view>
						</view>
					</view>
					<view class="input-li">
						<view class="input-label">* 紧急联系人电话：</view>
						<view class="input-box">
							<u-input placeholder="请输入" :border="'surround'" fontSize="24rpx"
								v-model="formObj.urgent_mobile"></u-input>
						</view>
					</view>
		...
		<simpleAddress ref="simpleAddress" :show="addresShow" :pickerValueDefault="cityValueDefault"
			@onConfirm="onConfirm">
		</simpleAddress>
	...
	export default {
		...
		methods: {
			...
			formReg() {
				...
				if (telReg.test(this.formObj.mobile) == false || telReg.test(this.formObj.urgent_mobile) == false) {
