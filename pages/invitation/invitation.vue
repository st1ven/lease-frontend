<template>
	<view style="background-color: #f6f6fb;">
		<view class="invitation u-skeleton" :style="{ backgroundImage: 'url(' + configg.background_image + ')' }"
			style="padding:  0 0 20px 0;">
			<view class="huodonganniu">
				<text @click="huodongguize" class="huodongbu"
					:style="{ background: configg.page_json.ActivityB, color: configg.page_json.ActivityC, boxShadow: configg.page_json.ActivityS }">{{
						configg.page_json.Activity }}</text>
				<text @click="jianglijiludingyue" class="huodongbu"
					:style="{ background: configg.page_json.RewardB, color: configg.page_json.RewardC, boxShadow: configg.page_json.RewardS }">{{
						configg.page_json.Reward }}</text>
			</view>
			...
			<view class="" style="padding: 10px;">
				...
				<view class="widthdrow">
					...
					<text v-if="tokenstatus == ''" class="tixiantext"
						style="background: linear-gradient(179deg, #8b8b8b 0%, #cdcdcd 100%);" @click="qingdenglu">提现到支付宝</text>
					<button v-else type="default" class="tixiantext" open-type="getPhoneNumber" @getphonenumber="decryptPhoneNumber"
						:style="{ background: configg.page_json.ButtonC }"
				...
				<view class="yaoqingtishi">
					<text class="widthdrowtext" :style="{ background: configg.page_json.ButtonC }">每邀请一名用户成功租机，您可以获得{{ configg.price
					}}元现金</text>
					<view class="da">
						<!-- #ifdef MP-ALIPAY -->
						<scroll-view scroll-x="true" class="flex">
							<view class="four">
								...
								<view class="fourthree" v-for="(item, index) in userinfo.useryao" :key="index">
...
export default {
	...
	onLoad(e) {
		...
		console.log(this.tokenstatus, 'tokenstatus')
		this.shareconfig()
		this.tokenexpir()
		// #ifdef H5
		this.userinfoget()
