	...
	<view class="u-upload" :style="[$u.addStyle(customStyle)]">
		<view class="u-upload__wrap" v-if="previewImage">
			<view class="u-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
				...
				<view class="u-upload__deletable" v-if="item.status !== 'uploading' && (deletable || item.deletable)"
			...
			<template v-if="isInCount">
				<view v-if="$slots && ($slots.default || $slots.$default)" @tap="chooseFile">
					<slot />
				</view>
			<!-- 	<view v-if=" $slots.default || $slots.$default " @tap="chooseFile">
				...
				<!--   $slots.default报错的问题-->
				<!-- <view v-if="$slots" @tap="chooseFile">
					<slot />
				</view> -->
